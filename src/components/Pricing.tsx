        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-[var(--color-border)]">
          {pricingPlans.map((plan, i) => (
            <ScrollReveal key={plan.name} delay={i * 100}>
              <div
                className={`p-8 md:p-10 transition-colors duration-100 ${
                  plan.highlighted
                    ? "bg-[var(--color-foreground)] text-[var(--color-background)] lg:-mt-6 lg:-mb-6 relative z-10 h-full"
                    : "bg-[var(--color-background)] hover:bg-[var(--color-foreground)] hover:text-[var(--color-background)] h-full"
                }`}
              >
                ...
                <Link
                  ...
                  className={`block text-center py-4 text-sm font-[family-name:var(--font-mono)] uppercase tracking-widest transition-all duration-100 ${
                    plan.highlighted
                      ? "bg-[var(--color-background)] text-[var(--color-foreground)] border-2 border-transparent hover:bg-transparent hover:text-[var(--color-background)] hover:border-[var(--color-background)]"
